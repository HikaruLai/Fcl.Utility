...
namespace Fcl.Utility
{
    public class HexConverter : IHexConverter
    {
        ...
        /// </summary>
        private static readonly int hexPerByte = 2;

        private static readonly string[] hexArr =
        {
        ...
        private static Dictionary<string, byte> byteDic { get; set; }

        public HexConverter(string encode = "UTF-8")
        {
            byteDic = hexArr.ToDictionary(item => item, item => Convert.ToByte(item, 16));
        ...
        {       
            ...
            {
                throw new Exception(String.Format("Hex string {0}, length should be even", hexStr));
            }
            //byte[] length: hexStr/2
            byte[] byteArr = new byte[hexStr.Length / hexPerByte];
            for (int i = 0; i < hexStr.Length; i = i + hexPerByte)
            {
                //each 2 Num(16) to Num(10)
                byteArr[i / hexPerByte] = byteDic[hexStr.Substring(i, hexPerByte)];
            }
            return byteArr;
        }
